...
void init_GPIO_TIM3(uint16_t dutycycle)                                 // Initialisation du TIM3 pour faire sonner le Buzzer, le MiniMoteur et les servo moteurs
{
 ...
 // TIM4->CR1|= TIM_CR1_ARPE;                        // On active l'auto-reload
  ...
  TIM4->PSC = 8;     //on est a 100kH                                // On divise la fréquence de la clk de la carte (avant = 439)
  TIM4->ARR = 10000;     //on compte 500 coup dhorloge               // On choisi la valeur à laquelle notre compteur doit aller (avant = 39) 4990
 // TIM4->CCR4= 100;     // regle le rapport cyclique 4950
  TIM4->CCR3= dutycycle;
// il faut être entre 550 et 2330 us
}
...
void init_PWM_MOT1(void)                                  // Initialisation du PWM pour faire tourner le Servo Moteur 1 sur le TIM3_CH3 sur PC8
{
 ...
 // TIM3->CCR1 = 100;                                       // On choisie la valeur de comparaison. Le signal passe a 1 quand le compteur dépasse cette valeur
...
{ 
  ...
  GPIOB->MODER |=   GPIO_MODER_MODER8_1;                 // On configure le MODER8 (car le minimot est sur PC8) en alternate function pour accéder au TIM3 sur AF2
  GPIOB->MODER &= ~ GPIO_MODER_MODER8_0;                 // On configure le MODER8 (car le minimot est sur PC8) en alternate function pour accéder au TIM3 sur AF2              
  ...
  GPIOB->AFR[1] |= (2<<0);                               // Registre de séléction de la fonction multiplexée
                                                         // AFR[1] le crochet 1 = AFRL HIGH car on est sur PC8 (AFRH8). PC8 donc sur les bits 0 à 3.
                                                         // On est sur TIM3 ==> AF2 ==>0010 ==> donc on met le bit 1 à 1 car par defaut on est a 0
}
...
void init_MOT2(void)                                      // Initialisation pour faire tourner le Servo Moteur 2
{
  ...
  GPIOB->MODER |=   GPIO_MODER_MODER9_1;                 // On configure le MODER9 (car le minimot est sur PC9) en alternate function pour accéder au TIM3 sur AF2
  GPIOB->MODER &= ~ GPIO_MODER_MODER9_0;                 // On configure le MODER9 (car le minimot est sur PC9) en alternate function pour accéder au TIM3 sur AF2              
  GPIOB->OTYPER &= ~ GPIO_OTYPER_OT_9;                   // On configure en mode push-pull
  GPIOB->AFR[1] |= (1<<5);                              // Registre de séléction de la fonction multiplexée
                                                         // AFR[1] le crochet 1 = AFRL HIGH car on est sur PC9 (AFRH9). PC9 donc sur les bits 4 à 7.
                                                         // On est sur TIM3 ==> AF2 ==>0010 ==> donc on met le bit 5 à 1 car par defaut on est a 0
}*/
